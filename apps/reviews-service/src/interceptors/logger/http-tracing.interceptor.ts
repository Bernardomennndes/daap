...
import { ILoggerService } from "../../lib/global/logger/adapter";
import { Observable, tap } from "rxjs";
import { FORMAT_HTTP_HEADERS, Span, SpanOptions, Tags } from "opentracing";
import { TracingType } from "../../lib/utils/request";
...
export class TracingInterceptor implements NestInterceptor {
  ...
  intercept(
    ...
    return next.handle().pipe(
      tap(() => {
        request.tracing.setTag(Tags.HTTP_STATUS_CODE, response.statusCode);
