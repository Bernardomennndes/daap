...
export class LoggerService implements ILoggerService {
  pino: HttpLogger;
  private app: string;
  private streamToElastic: Transform | null;
  ...
  constructor(private readonly elkUrl: string) {
    ...
    try {
      ...
      console.warn('Failed to initialize Elasticsearch stream:', error.message);
      this.streamToElastic = null;
    }
  }
  ...
  connect<T = LevelWithSilent>(logLevel: T): void {
    ...
    // Only add Elasticsearch stream if it was successfully initialized
    if (this.streamToElastic) {
      streams.push({ level: 'info', stream: this.streamToElastic });
    }
